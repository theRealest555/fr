    ...
    <div class="mb-4 transition-all duration-150 hover:shadow-sm">
      <label [for]="id" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1 items-center">
        ...
        <div *ngIf="tooltip" class="relative ml-2 inline-block group">
          ...
          <div class="absolute z-10 transform -translate-x-1/2 left-1/2 mt-2 w-64 px-3 py-2 bg-gray-900 dark:bg-dark-700 text-white dark:text-gray-200 text-xs rounded-md shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200">
            {{ tooltip }}
      ...
      <!-- Error messages -->
      <div *ngIf="shouldShowError()" class="mt-1 text-sm text-red-600 dark:text-red-500 transition-all duration-150 animate-fade-in">
      ...
      <!-- Hint text when no errors -->
      <div *ngIf="hint && !shouldShowError()" class="mt-1 text-xs text-gray-500 dark:text-gray-400">
        {{ hint }}
      </div>
    </div>
  `,
